// Module
export default {

  title: 'Help Center',
  searchPlaceholder: 'Search questions...',
  faq: 'FAQ',
  gameGuide: 'Game Guide',
  pointsRules: 'Points Rules',
  contactUs: 'Contact Us',
  onlineService: {
    title: 'Online Service',
    serviceHours: 'Service Hours: 9:00 - 22:00',
    startChat: 'Start Chat',
    otherContactMethods: 'Other Contact Methods',
    customerServicePhone: 'Customer Service Phone',
    customerServiceEmail: 'Customer Service Email',
    workingHours: 'Working Hours',
    workingHoursValue: 'Monday to Sunday 9:00-22:00',
    averageResponseTime: 'Average response time: 2 minutes',
    clearHistory: 'Clear History',
    startChatHint: 'Start chatting with customer service',
    inputPlaceholder: 'Please enter your question...',
    send: 'Send',
    onlineStatus: 'Online Service',
    offlineStatus: 'Offline Status',
    welcomeMessage: 'Hello! Welcome to online customer service, how can I help you today?',
    typingMessage: 'Customer service is typing...',
    clearHistoryMessage:
      'Are you sure you want to clear all chat history? This operation cannot be undone.',
    clearHistorySuccess: 'Chat history has been cleared',
    pageLoaded: 'Online service page loaded',
    quickQuestions: {
      rechargePoints: 'How to recharge points?',
      cashExchange: 'Can points be exchanged for cash?',
      forgotPassword: 'What if I forget my password?',
      modifyProfile: 'How to modify personal information?',
      viewGameRecords: 'Where to view game records?',
    },
    faq: {
      howToEarnPoints: 'How to earn points?',
      howToEarnPointsAnswer:
        'You can earn points through daily check-ins, participating in games, inviting friends, etc. Please check the instructions for each game for specific rules.',
      pointsExpire: 'Do points expire?',
      pointsExpireAnswer:
        'Points are permanently valid and do not expire. Please keep your account secure.',
      howToContact: 'How to contact customer service?',
      howToContactAnswer:
        'You can contact us through online customer service, customer service phone 400-123-4567, or customer service email ({email}).',
    },
    replies: {
      points:
        'Regarding points, you can earn points through daily check-ins, participating in games, inviting friends, etc. For recharge, you can operate on the "Points Exchange" page.',
      account:
        'To modify your password or retrieve your account, please go to the "Account Settings" page. If you encounter any issues, please provide your user ID and we will assist you.',
      games:
        'We offer a variety of interesting games, including Number Guess, Lucky Wheel, Quiz Challenge, etc. You can check detailed game rules in the "Help Center".',
      thanks:
        "You're welcome! If you have any other questions, feel free to ask. Have a great time gaming! 😊",
      default1: 'Thank you for your inquiry, we are processing your question.',
      default2:
        'Received your question, please wait a moment while I look up the information for you.',
      default3: 'Okay, I understand, let me explain in detail for you.',
      default4: 'I need to confirm this question, please wait a moment.',
      default5:
        'Sorry to keep you waiting, regarding your question, we recommend checking the detailed instructions in the Help Center.',
    },
  },
  feedback: {
    title: 'Feedback',
    description: 'Your feedback is very important to us. Please tell us how we can improve the product experience.',
    type: 'Feedback Type',
    typeOptions: {
      featureSuggestion: 'Feature Suggestion',
      bugReport: 'Bug Report',
      experienceImprovement: 'Experience Improvement',
      contentIssue: 'Content Issue',
      other: 'Other',
    },
    descriptionLabel: 'Problem Description',
    descriptionPlaceholder: 'Please describe your issue or suggestion in detail, including:\n1. What is the specific problem?\n2. Under what circumstances did it occur?\n3. What improvement do you expect...',
    contactLabel: 'Contact Information (Optional)',
    contactPlaceholder: 'Phone number or email for us to contact you',
    uploadImages: 'Upload Screenshots (Optional)',
    uploadPlaceholder: 'Upload Image',
    uploadTip: 'Max 3 images, jpg and png formats supported',
    submit: 'Submit Feedback',
    submitting: 'Submitting...',
    myFeedback: 'My Feedback',
    noFeedback: 'No feedback records',
    feedbackDetail: 'Feedback Detail',
    feedbackType: 'Feedback Type',
    reply: 'Official Reply',
    status: {
      pending: 'Pending',
      processing: 'Processing',
      resolved: 'Resolved',
      closed: 'Closed',
    },
    validation: {
      selectType: 'Please select feedback type',
      enterDescription: 'Please enter problem description',
      descriptionLength: 'Description requires at least 10 characters',
    },
    success: {
      submit: 'Feedback submitted successfully! We will process your suggestion soon.',
      thanks: 'Thank you for your valuable feedback!',
    },
  },
  report: {
    title: 'Report',
    description: 'If you find any violations or inappropriate content, please report it to us immediately and we will handle it as soon as possible.',
    type: 'Report Type',
    typeOptions: {
      violation: 'Violation',
      fraud: 'Fraud',
      cheating: 'Cheating',
      harassment: 'Harassment',
      other: 'Other',
    },
    target: 'Reported Object',
    targetPlaceholder: 'Please enter username, ID or related link',
    reason: 'Report Reason',
    reasonPlaceholder: 'Please describe the reason for reporting in detail, including:\n1. What is the specific violation?\n2. When and where did it happen?\n3. Evidence description...',
    contactLabel: 'Contact Information (Optional)',
    contactPlaceholder: 'Phone number or email for us to contact you',
    uploadEvidence: 'Upload Evidence (Optional)',
    uploadPlaceholder: 'Upload Image',
    uploadTip: 'Max 5 images, jpg and png formats supported',
    submit: 'Submit Report',
    submitting: 'Submitting...',
    myReports: 'My Reports',
    noReports: 'No report records',
    reportDetail: 'Report Detail',
    reportType: 'Report Type',
    processedResult: 'Processed Result',
    status: {
      pending: 'Pending',
      processing: 'Processing',
      resolved: 'Resolved',
      rejected: 'Rejected',
    },
    validation: {
      selectType: 'Please select report type',
      enterTarget: 'Please enter reported object',
      enterReason: 'Please enter report reason',
      reasonLength: 'Report reason requires at least 10 characters',
      saveFailed: 'Save failed, please clear some history records and try again',
    },
    success: {
      submit: 'Report submitted successfully! We will verify and process it soon.',
      thanks: 'Thank you for your supervision!',
    },
  },
  howToEarn: 'How to earn points?',
  howToUse: 'How to use points?',
  expireRule: 'Do points expire?',
  guidePopup: {
    rules: 'Game Rules',
    rewards: 'Reward Mechanism',
    notes: 'Notes',
  },
  pointsPopup: {
    title: 'Points Rules',
    earn: {
      title: 'How to Earn Points',
      items: [
        'Daily check-in: 10 base points, with additional rewards for consecutive check-ins',
        'Number Guess: 10-50 points based on accuracy',
        'Lucky Wheel: Cost 10 points to participate, chance to win higher rewards',
        'Quiz Challenge: 5 points per correct answer, additional rewards for consecutive correct answers',
        'Invite friends: 50 points per invited friend',
      ],
    },
    use: {
      title: 'How to Use Points',
      items: [
        'Participate in Lucky Wheel games (costs 10 points each time)',
        'Exchange for gifts in the points exchange center',
        'Unlock special game features',
        'Purchase game props',
      ],
    },
    expire: {
      title: 'Points Validity',
      items: [
        'Points are permanently valid and do not expire',
        'Please keep your account secure to avoid point loss',
        'If you find any point abnormalities, please contact customer service promptly',
      ],
    },
  },
  faqList: [
    {
      question: 'How to start playing games?',
      answer:
        'Click the "Games" button in the bottom navigation bar to enter the game center, select the game you are interested in to start. Each game has detailed game rules.',
    },
  ],
  gameGuides: {
    numberGuess: {
      rules:
        'The system randomly generates a number between 1-100, you have 7 attempts to guess. After each guess, you will be told if the number is "too big" or "too small".',
      rewards:
        '50 points for guessing within 7 attempts, 30 points within 5 attempts, 10 points within 3 attempts.',
      notes:
        'You can participate unlimited times per day, but only the first victory can earn points.',
    },
    luckyWheel: {
      rules:
        'Cost 10 points to spin the wheel, which has 6 regions corresponding to different prize levels.',
      rewards:
        'First prize 500 points, second prize 200 points, third prize 100 points, fourth prize 50 points, fifth prize 20 points, thank you for participating.',
      notes: 'The wheel uses precise positioning algorithm to ensure fairness.',
    },
    dailyCheckIn: {
      rules: 'Check in every day to get basic point rewards.',
      rewards:
        'Basic reward 10 points, additional +5 points for 3 consecutive days, +15 points for 7 days, +50 points for 30 days.',

    },
  },
  userGuide: 'User Guide',
  gameRules: 'Game Rules',
  accountHelp: 'Account Issues',
  paymentHelp: 'Payment Issues',
  technicalSupport: 'Technical Support',
  serviceHours: 'Service Hours: 9:00-21:00',
  emailUs: 'Send Email',
  callUs: 'Make Call',
  followUs: 'Follow Us',
  submitFeedback: 'Submit Feedback',
  feedbackType: 'Feedback Type',
  feedbackContent: 'Feedback Content',
  contactInfo: 'Contact Information (Optional)',
  pleaseSelectType: 'Please select feedback type',
  pleaseEnterContent: 'Please enter feedback content',
  feedbackSubmitted: 'Feedback submitted successfully',
  reportType: 'Report Type',
  reportReason: 'Report Reason',
  evidence: 'Evidence Screenshot',
  reportSubmitted: 'Report submitted successfully',
  chatWithService: 'Chat with Service',
  waitingForService: 'Waiting for service...',
  serviceConnected: 'Service connected',
  typeMessage: 'Type message...',
  send: 'Send',
  // 组件通用文本
  components: {
    myRecords: 'My Records',
    noRecords: 'No records',
    detail: 'Detail',
    type: 'Type',
    target: 'Target',
    description: 'Description',
    reply: 'Reply',
    images: 'Images',
    submitTime: 'Submit Time',
    processStatus: 'Process Status',
    contact: 'Contact',
  },

}
