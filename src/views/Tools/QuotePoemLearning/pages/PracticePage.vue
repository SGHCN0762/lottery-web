  ...
  <div class="practice-page">
    ...
    <div v-else class="practice-content">
      ...
      <div v-if="!practiceStarted" class="start-screen">
        ...
        <p class="start-desc">选择练习类型，巩固所学知识</p>
...
const { allItems, loading, markMastered, recordPractice } = useQuotePoemLearning();
  ...
  for (let i = 0; i < questionCount; i++) {
    ...
    if (selectedType.value === 'choice') {
      ...
      if (keySentences.length > 0) {
        ...
        // 名言题
        const correctAnswer = item.author;
        const otherItems = stageItems.filter(s => s.id !== item.id && s.type === 'quote');
        const wrongOptions = otherItems
          .sort(() => Math.random() - 0.5)
          .slice(0, 3)
          .map(s => s.author);
        ...
        questionList.push({
          type: 'choice',
          question: `"${truncateText(item.content, 30)}"这句名言的作者是？`,
          options,
          correctIndex,
          correctAnswer,
          author: item.author,
      ...
      if (keySentences.length > 0) {
        ...
        if (words.length >= 2) {
          ...
          if (blankWord.length >= 2) {
            const question = sentence.replace(blankWord, '______');
            questionList.push({
              type: 'fillBlank',
              question,
