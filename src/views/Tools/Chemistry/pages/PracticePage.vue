  ...
  <div class="practice-page">
    <div class="practice-content" v-if="!isFinished">
      <div class="progress-section">
        ...
        <div class="progress-info">
          ...
          <span class="score-text">{{ t('tools.chemistry.practice.score') }}: {{ score }}</span>
        </div>
      </div>
      ...
      <div class="question-card" v-if="currentQuestion">
        <div class="question-header">
          <span class="question-index">
            {{ t('tools.chemistry.practice.question') }} {{ currentQuestionIndex + 1 }}
          </span>
          <span class="question-type">{{ t('tools.chemistry.practice.choice') }}</span>
        </div>
        ...
        <div v-if="showResult" class="explanation-section">
          <h4 class="explanation-title">{{ t('tools.chemistry.practice.explanation') }}</h4>
      ...
      <div class="action-buttons">
        <van-button
          ...
          class="submit-btn"
        >
          {{ t('tools.chemistry.practice.submit') }}
        </van-button>
        <van-button
          v-else
          type="primary"
          ...
          class="next-btn"
        >
          {{ currentQuestionIndex === totalQuestions - 1 ? t('tools.chemistry.practice.finish') : t('tools.chemistry.practice.next') }}
        </van-button>
      </div>
    </div>
    ...
    <div class="result-content" v-else>
      <div class="result-card">
        ...
        <h2 class="result-title">{{ t('tools.chemistry.practice.completed') }}</h2>
        ...
        <div class="result-stats">
          <div class="stat-item">
            ...
            <span class="stat-label">{{ t('tools.chemistry.practice.correct') }}</span>
          </div>
          <div class="stat-item">
            <span class="stat-value wrong">{{ totalQuestions - score }}</span>
            <span class="stat-label">{{ t('tools.chemistry.practice.wrong') }}</span>
          </div>
          <div class="stat-item">
            <span class="stat-value rate">{{ Math.round((score / totalQuestions) * 100) }}%</span>
            <span class="stat-label">{{ t('tools.chemistry.practice.rate') }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="result-actions">
        <van-button type="default" @click="$router.back()" class="back-btn">
          {{ t('tools.chemistry.actions.back') }}
        </van-button>
        <van-button type="primary" @click="retry" class="retry-btn">
          {{ t('tools.chemistry.practice.retry') }}
        </van-button>
      </div>
    </div>
  </div>
...
import { useChemistryPractice } from '../hooks/useChemistry';
...
const resultDesc = computed(() => {
  const rate = totalQuestions.value > 0 ? score.value / totalQuestions.value : 0;
  if (rate >= 0.8) return t('tools.chemistry.practice.excellent');
  if (rate >= 0.6) return t('tools.chemistry.practice.good');
  if (rate >= 0.4) return t('tools.chemistry.practice.pass');
  return t('tools.chemistry.practice.needWork');
});
