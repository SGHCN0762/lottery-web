    ...
    <div v-else class="learn-content">
      ...
      <div v-if="!learnStarted" class="start-screen">
        ...
        <div class="stage-selector">
          ...
          >
            ...
            <span class="stage-count">{{ getStageCount(key) }} 篇</span>
          </div>
        </div>
      ...
      <div v-else class="learn-card-container" v-if="currentItem">
        <div class="learn-card">
          <div class="card-type-badge">文言文</div>

          <div class="card-title">
            {{ currentItem.title }}
            <span class="title-author">{{ currentItem.dynasty }} · {{ currentItem.author }}</span>
          </div>
          ...
          <div class="tab-content">
            ...
            <div v-if="activeTab === 'vocabulary'" class="tab-panel">
              <div v-if="currentItem.vocabulary && currentItem.vocabulary.length > 0" class="vocabulary-list">
                <div
                  v-for="(word, idx) in currentItem.vocabulary"
                  :key="idx"
              ...
              <div v-else class="panel-empty">暂无重点字词</div>
            </div>
          </div>
        </div>
...
const { allItems, loading, markMastered, isMastered } = useClassicalEssays();
...
const availableTabs = computed(() => {
  if (!currentItem.value) return [];
  const tabs = [];
  if (currentItem.value.translation) {
  ...
  if (currentItem.value.vocabulary && currentItem.value.vocabulary.length > 0) {
    tabs.push({ key: 'vocabulary', name: '字词' });
  }
  return tabs;
});
...
.vocabulary-item {
  ...
  gap: var(--spacing-md);
  padding: var(--spacing-sm) var(--spacing-md);
  background: var(--van-purple-50);
  border-radius: var(--radius-md);
  border-left: 3px solid var(--van-purple-500);
}

.word-text {
  font-size: var(--font-size-sm);
  font-weight: var(--font-weight-semibold);
  color: var(--van-purple-600);
  flex-shrink: 0;
}
