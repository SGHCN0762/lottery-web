    ...
    >
      <div class="chat-window">
        <div class="chat-header">
          ...
          <van-button size="mini" type="primary" plain @click="clearChatHistory">
        ...
        <div class="chat-messages" ref="messagesRef">
          ...
          <div v-for="msg in chatMessages" :key="msg.id" class="message-item" :class="msg.type">
            <div v-if="msg.type === 'system'" class="system-message">
              {{ msg.content }}
            </div>
            <div v-else class="message-content">
              <div class="message-avatar">
                <van-icon :name="msg.type === 'user' ? 'user-o' : 'service'" size="20" />
              </div>
...
<script setup>
  ...
  const serviceStatusText = computed(() => {
    return isServiceTime.value
      ? t('help.onlineService.onlineStatus')
      : t('help.onlineService.offlineStatus');
  });
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      ...
      chatMessages.value = chatMessages.value.filter(
        msg => msg.content !== t('help.onlineService.typingMessage')
      );

      // 智能回复逻辑
      let reply = '';
      if (
        userMessage.includes('积分') ||
        userMessage.includes('充值') ||
        userMessage.includes('points') ||
        userMessage.includes('recharge')
      ) {
        reply = t('help.onlineService.replies.points');
      } else if (
        userMessage.includes('密码') ||
        userMessage.includes('账号') ||
        userMessage.includes('password') ||
        userMessage.includes('account')
      ) {
        reply = t('help.onlineService.replies.account');
      } else if (
        userMessage.includes('游戏') ||
        userMessage.includes('玩法') ||
